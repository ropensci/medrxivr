test_that("Error handling", {
  expect_error(mx_reporter())
  expect_error(mx_reporter(data = 2))
})
...
mx2 <-
  mx_reporter(mx_data, query = c("dementia"), to_date = "2020-01-01")
mx3 <-
  mx_reporter(mx_data, query = list("dementia"), to_date = "2020-01-01")
...
mx4 <- mx_reporter(mx_data, query = c("dementia", "lipid"))
mx5 <- mx_reporter(mx_data, query = list("dementia", "lipid"))
...
mx6 <- mx_reporter(mx_data, query = "dementia", deduplicate = TRUE)
mx7 <- mx_reporter(mx_data, query = "dementia", deduplicate = FALSE)
...
mx8 <- mx_reporter(mx_data, query = "dementia", NOT = "[Dd]ementia")

test_that("NOT", {
  skip_if_offline()
  expect_message(mx_reporter(mx_data, query = "dementia", NOT = "dementia"),
                 regexp = "No records found"
  )
})

mx9 <-
  mx_reporter(mx_data, query = list("dementia", "Alz", "vascular"))
mx10 <-
  mx_reporter(mx_data, query = list("dementia", "Alz", "vascular", "sex"))
mx11 <-
  mx_reporter(mx_data, query = list(
