...
- `mx_snapshot()` provides access to a static snapshot of the medRxiv
  database from the [medrxivr-data
  ...
  usage times) and is usually faster (as it reads data from CSV files
  rather than having to re-extract it from the API). The function prints
  the latest record date included in the snapshot; discrepancies between
  the most recent static snapshot and the live database can be assessed
  using `mx_crosscheck()`.
...
See here for the [snapshot data
